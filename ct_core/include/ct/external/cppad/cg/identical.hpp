...
template<class Base>
inline bool IdenticalPar(const CppAD::cg::CG<Base>& x) noexcept (false) {
    if (!x.isParameter()) {
...
template<class Base>
inline bool IdenticalZero(const CppAD::cg::CG<Base>& x) noexcept (false) {
    if (!x.isParameter()) {
        return false; // its value may change after tapping
    }
...
template<class Base>
inline bool IdenticalOne(const CppAD::cg::CG<Base>& x) noexcept (false) {
    if (!x.isParameter()) {
        return false; // its value may change after tapping
    }
